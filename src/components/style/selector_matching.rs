...
fn matches_simple_selector<N: TreeNode<T>, T: TreeNodeRefAsElement<N, E>, E: ElementLike>(
    ...
    //println(fmt!("In matches_simple_selector(), *element = %?", *element));
    ...
    match *selector {
        ...
        FirstChild => matches_pseudo_class(selector, element),
...
#[inline]
fn matches_pseudo_class<N: TreeNode<T>, T: TreeNodeRefAsElement<N, E>, E: ElementLike>(
        selector:&SimpleSelector, element: &T) -> bool {
    match *selector {
        FirstChild => {
            let mut r = false;
